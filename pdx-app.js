...
function isOpen(day){ return day <= highestDone() + 1; }
...
function vLesson(day){
  ...
  quizState = { day: day, picked: [null, null, null, null, null], essay: done && done.essayText ? done.essayText : "", turnedIn: !!done };
...
function renderQuiz(){
  ...
  e.questions.forEach(function (q, qi) {
    ...
    q.choices.forEach(function (c, ci) {
      ...
      } else if (st.picked[qi] === ci) cls += " selected";
      html += '<button class="' + cls + '" data-q="' + qi + '" data-c="' + ci + '"' +
        (st.turnedIn ? " disabled" : "") + ">" +
        '<span class="letter">' + "ABCD"[ci] + "</span>" + esc(c) + "</button>";
    });
    html += "</div>";
  });
  if (e.essay) {
    html += '<div class="card"><p class="q-text">Essay — ' + esc(e.essay) + "</p>" +
      '<textarea class="essay" id="essay-box" placeholder="Write your essay here..."' +
      (st.turnedIn ? " disabled" : "") + ">" + esc(st.essay) + "</textarea>" +
  ...
  box.querySelectorAll(".choice").forEach(function (btn) {
    btn.addEventListener("click", function () {
      if (quizState.turnedIn) return;
      quizState.picked[parseInt(btn.getAttribute("data-q"), 10)] = parseInt(btn.getAttribute("data-c"), 10);
      renderQuiz();
      var eb = document.getElementById("essay-box");
      if (eb) eb.value = quizState.essay;
    });
  });
  var eb = document.getElementById("essay-box");
