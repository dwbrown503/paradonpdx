...
function vDoc(which){
  ...
  view.innerHTML = '<a class="btn secondary small" href="' + b + '">Back</a>' +
...
function vLessonDoc(day){
  ...
  var quizBtn = (window.DAYS && DAYS[day])
    ? '<a class="btn secondary small" href="#/learn/day/' + day + '">Take the Day ' + day + " quiz</a>" : "";
  view.innerHTML = '<a class="btn secondary small" href="#/guide">Written guide</a>' + quizBtn +
    '<div class="docview">' + body + "</div>";
}
...
function vLesson(day){
  ...
  quizState = { day: day, picked: [null, null, null, null, null], essay: done && done.essayText ? done.essayText : "", turnedIn: !!done, qi: 0 };
...
function renderQuiz(){
  ...
  var html = '<div class="card"><p class="q-text">How this works</p>' +
    '<p>Read the lesson first. Then answer the questions one at a time &mdash; ' +
    'tap your answer and you will move to the next question. ' +
    'Answer all ' + n + ' questions to turn in.</p></div>';
  if (qi < n) {
    var q = e.questions[qi];
    html += '<p class="muted center">Question ' + (qi + 1) + ' of ' + n + '</p>' +
      '<div class="card q-block"><p class="q-text">' + (qi + 1) + '. ' + esc(q.q) + '</p>';
    q.choices.forEach(function (c, ci) {
      html += '<button class="choice" data-c="' + ci + '"><span class="letter">' + 'ABCD'[ci] + '</span>' + esc(c) + '</button>';
    });
    html += '</div>';
    if (qi > 0) html += '<div class="btn-row"><button class="btn secondary" id="qback">\u2039 Back</button></div>';
  } else if (e.essay) {
  ...
  if (back) back.addEventListener("click", function () { st.qi = qi - 1; renderQuiz(); window.scrollTo(0, 0); });
